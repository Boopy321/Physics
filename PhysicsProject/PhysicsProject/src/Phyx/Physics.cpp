...
#include "Assets\Particle\ParticleEmitter.h"
#include "Assets\Particle\ParticleFluidEmitter.h"
...
Physics::Physics()
{
	...
	if (pf)
	{
		...
		m_particleEmitter = new ParticleEmitter(maxParticles,
			PxVec3(0, 10, 0), pf, .01f);
		m_particleEmitter->setStartVelocityRange(-2.0f, 0.0f, -2.0f,
			2.0f, 0.0f, 2.0f);
	}
}
...

{
	...
	PxRigidDynamic* dynamicActor = PxCreateDynamic(*g_Physics, PxTransform(PxVec3(10, 10, 0)), capsule,
	...
	PxBoxGeometry box5(5, 5, 5);
	PxRigidStatic* staticActor = PxCreateStatic(*g_Physics, PxTransform(PxVec3(10, 10, 0)), box5,
		*g_PhysicsMaterial);
